...
* Autor MhankBarBar, Farhan
* Adicione Cape Bro, correções que não funcionam
* Jan está no nome do meu doank
- O que há de novo?
...
const { m01 } = require('./lib/m01')
const { m02 } = require('./lib/m02')
const { m03 } = require('./lib/m03')
const { m04 } = require('./lib/m04')
const { m05 } = require('./lib/m05')
const { m06 } = require('./lib/m06')
const { m07 } = require('./lib/m07')
const { m08 } = require('./lib/m08')
const { m09 } = require('./lib/m09')
const { m10 } = require('./lib/m10')
const { m11 } = require('./lib/m11')
const { m12 } = require('./lib/m12')
const { m13 } = require('./lib/m13')
const { m14 } = require('./lib/m14')
const { m15 } = require('./lib/m15')
const { m16 } = require('./lib/m16')
const { m17 } = require('./lib/m17')
const { m18 } = require('./lib/m18')
const cd = 4.32e+7
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Bot Mk\n' // MUDE O NOME 
            + 'ORG:MK MODS;\n'
            + 'TEL;type=CELL;type=VOICE;waid=554499013362:+55 44-9901-3362\n' // ALTERAR NÚMERO 
            + 'END:VCARD'
       
prefix = "."
name = "BOT MK"
rdaftar = "OBRIGADO POR SE REGISTAR PARA SER UM USUÁRIO DO MK 😁"
rmenu = "Oi amigo 👋🏻"
botinfo = "PARA BOTS CONVIDADOS, TODOS BOTS TERMUX SAO INDONÉSIO NENHUM BR CRIA SÓ EDITA:)"
limitt = 999999999
memberLimit = 2
ban = []
premium = ["5544999013362@s.whatsapp.net"]
		...
		if(antifake.includes(anu.jid)) {
			if (anu.action == 'add'){
				...
				if(!num.split('@')[0].startsWith(55)) {
					denz.sendMessage(mdata.id, 'Corra numero fake seu Ban esta próximo!', MessageType.text)
...
🚫 Proibido links que nao seja do grupo.

🚫 Proibido desrespeitar participante do grupo.️

✔️ *Leia a descrição do grupo*`
	...
	denz.on('chat-update', async (mek) => {
		try {
			...
			const DnsBot = ["0@s.whatsapp.net"] // apenas mude, está tudo bem
			...
			const ownerNumber = ["5544999013362@s.whatsapp.net"] // apenas mude o número do dono
		...
		setTimeout( () => {
		reply(`Desculpe bot deixando o Grupo em breve *${groupMetadata.subject}*`)
		}, 0)
		}
        ...
        if (adminNumber.includes(messagesLink)){
        	hasil = `        ────────────────
Ola *${pushname2}* por favor digite*${prefix}menu*
        ────────────────`
denz.sendMessage(from, hasil, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "ketik #menu kak", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
		...
		if (isGroupAdmins) return reply(`*${pushname2}* é o grupo de administração, você não será expulso `)
		denz.updatePresence(from, Presence.composing)
...
╙───々`
        denz.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/bemvindo.mp3')
        denz.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
            break
            case 'help':
...
denz.sendMessage(from, brando, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
    	    break
				...
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, menu1(prefix), text)
					break
				case 'menu2':
				if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, menu2(prefix), text)
					break
				case 'help':
				case 'menu':
				if (isBanned) return reply(ind.baned())
                ...
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m01(prefix), text)
					break
                case 'm02':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m02(prefix), text)
					break
                case 'm03':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m03(prefix), text)
					break
                case 'm04':
                case 'yx':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m04(prefix), text)
					break
                case 'm05':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m05(prefix), text)
					break
                case 'm06':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m06(prefix), text)
					break
                case 'm07':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m07(prefix), text)
					break
                case 'm08':
                case 'spotify':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m08(prefix), text)
					break
                case 'm09':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m09(prefix), text)
					break
                case 'm10':
                case 'tiktok':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m10(prefix), text)
					break
                case 'm11':
                case 'business':
               if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m11(prefix), text)
					break
                case 'm12':
                case 'youtube':
                case 'vanced':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m12(prefix), text)
					break
                case 'm13':
                case 'instagram':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m13(prefix), text)
					break
                case 'm14':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m14(prefix), text)
					break
                case 'm15':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m15(prefix), text)
					break
                case 'm16':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m16(prefix), text)
					break
                case 'm17':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m17(prefix), text)
					break					
                case 'm18':
                case 'figurinhas':
                if (isBanned) return reply(ind.baned())
				if (!isUser) return reply(mess.only.userB)
					denz.sendMessage(from, m18(prefix), text)
					break		
            case 'fig':
				case 'stiker':
				case 'sticker':
			...
			if (!tex) return ahmad.sendMessage(from, 'INSIRA O URL / TEXTO PARA FAZER QR', text, {quoted: mek})
...
break
               ...
               break
					...
					denz.sendMessage(from, atytyd, sticker, {quoted: amv})
					await limitAdd(sender) 
					break
                  case 'galaxstyle':
                   if (isBanned) return reply(mess.only.benned)
				...
				denz.sendMessage(from, ct, image, {caption: 'Aqui maninha ja terminou..', quoted: mek})
				await limitAdd(sender)
				break
            case 'attp':
			case 'attp2':
			    if (isBanned) return reply(mess.only.benned)
			        if (!isUser) return reply(mess.only.userB)    
				if (args.length < 1) return reply(`O texto é\n*Exemplo ${prefix}attp Sou Lindo*`)
				...
				case 'bomdia':
					...
					denz.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia, vcs sao top! ️'})
					break
				case 'boatarde':
				    if (isBanned) return reply(mess.only.benned)
			        if (!isUser) return reply(mess.only.userB)  
                    reply(mess.wait)
                    boatarde = body.slice(5)
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					denz.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa tarde pessoal 😎👍'})
					break
				case 'boanoite':
			        if (isBanned) return reply(mess.only.benned)
			        if (!isUser) return reply(mess.only.userB)           
                    reply(mess.wait)
                    boanoite = body.slice(5)
					buffer = await getBuffer(`https://i.imgur.com/yOFxSUR.jpg`)
					denz.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa noite fml 😴️'})
					break	
				case '3dtext':
				case '3dtexto':
				 if (isBanned) return reply(mess.only.benned)
                ...
                denz.sendMessage(from, data, image, {quoted: mek, caption: body.slice(8)})
                await limitAdd(sender)
                break
                case 'text3d':
                case 'texto3d':
                    if (isBanned) return reply(mess.only.benned)
			        if (!isUser) return reply(mess.only.userB)
             	   if (args.length < 1) return reply('qual texto lindx??')
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return denz.sendMessage(from, 'limite de 10 letras', text, {quoted: mek})
                    buff = await getBuffer(`https://docs-jojo.herokuapp.com/api/text3d?text=${teks}`, {method: 'get'})
                    denz.sendMessage(from, buff, image, {quoted: mek, caption: `${teks}`})
			     	break
					case 'shopee':
                      if (isBanned) return reply(mess.only.benned)
			        if (!isUser) return reply(mess.only.userB)
...
quoted: mek, caption: `Wah ganteng kek gua`
  ...
  break
			...
			if (!isUser) return reply(mess.only.userB)
				...
				case 'r':
					denz.updatePresence(from, Presence.composing)
					if (isUser) return reply(`*⚠️「 JÁ REGISTRADO 」⚠️*\n\n*você registrou >_<*`)
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
				    ...
				    captionnya =   `*✔️REGISTRO DE SUCESSO✔️*\nPara informações do usuário :\n\n*➸ Nome : ${pushname2}*\n*➸ Número : wa.me/${sender.split("@")[0]}*\n*➸ Idade : ${umurUser}*\n*➸ Hora do registro : ${time}*\n\n*「SN」: ${serialUser}*\nTotal de pessoas  ${user.length} usando\n_NOTA: Esta mensagem é muito importante._`
					brando = await getBuffer(`https://pencarikode.xyz/api/cita-cita?apikey=pais`, {method: 'get'})                
				...
				case 'ban':
			        ...
			        ban = mentioned
					reply(`😂 *HaHaHa acabou de levar Ban* 🚫👋 ${ban}`)
					break
					case 'tahta':
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           denz.sendMessage(from, options, text, { quoted: mek } )
				...
				case 'saibot': 
				    if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
			    	anu = await denz.groupLeave(from, `Tchau, todos os membros *${groupMetadata.subject}*`, groupId)
	                break
				case 'setname':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
  ...
  break
				...
				if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if ((args[0]) === '1') {
						...
						reply('1 para ligar, 0 para desabilitar')
					}
					break
			    case 'nsfw':
			    if (isBanned) return reply(mess.only.benned)    
                ...
                case 'delpalavra':
                    if (!isOwner) return reply(mess.only.ownerB)
                    if (args.length < 1) return reply( `Enviar pedidos ${prefix}delbadword [kata kasar]. contoh ${prefix}delbadword bego`)
...
case 'husbu':
			...
			break
				...
				if (budy.includes("://chat.whatsapp.com/")){
					if (!isGroup) return
					if (!isAntiLink) return
					if (isGroupAdmins) return reply(`*Publicação não Autorizado!* \n${pushname2} Você é um administrador do grupo, Bot não vou te chutar`)
					denz.updatePresence(from, Presence.composing)
					if (messagesLink.includes("#izinadmin")) return reply("#izinadmin *_「 ᴅɪᴛᴇʀɪᴍᴀ 」_*")
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
					reply(`⚠️ *Publicação não Autorizado!* ⚠️\n${sender.split("@")[0]} Seu ban chegando`)
		            setTimeout( () => {
			        denz.groupRemove(from, [kic]).catch((e)=>{reply(`O Bot Deve Ser Administrador`)})
		            }, 3000)
		        setTimeout( () => {
			        denz.updatePresence(from, Presence.composing)
			        reply("1 Segundo, Thau 👋")
		        }, 2000)
		        setTimeout( () => {
			        denz.updatePresence(from, Presence.composing)
			        reply("2 Segundos")
		        ...
		        setTimeout( () => {
		          ...
		          if (budy.includes("Ganhar dinheiro baixando o Kwai!")){
					if (!isGroup) return
					if (!isAntiLink) return
					if (isGroupAdmins) return reply(`*Publicação não Autorizado!* \n${pushname2} Você é um administrador do grupo, Bot não vou te chutar`)
					denz.updatePresence(from, Presence.composing)
			...
			denz.updatePresence(from, Presence.composing)
			reply("*${pushname2}* Teu pai")
				}

        	if (budy.includes("Bosta")){
			denz.updatePresence(from, Presence.composing)
	...
	if (budy.includes("Porno")){
			denz.updatePresence(from, Presence.composing)
			reply("*${pushname2}* Porno não é bem vindo no grupo, vai ser Banido.")
	}

	     	if (budy.includes("Corno")){
			denz.updatePresence(from, Presence.composing)
			reply("*${pushname2}* que é o Corno🐂")
	}
	
	    	if (budy.includes("Boiola")){
			denz.updatePresence(from, Presence.composing)
			reply("*${pushname2}* É tu!")
	}
	
	    	if (budy.includes("Sfd")){
			denz.updatePresence(from, Presence.composing)
			reply("Bora juntos?")
	}
	
	       if (budy.includes("Link")){
			denz.updatePresence(from, Presence.composing)
			reply("Link somente do Adm 👍")
	}
	
	    	if (budy.includes("Cadê o bot")){
			denz.updatePresence(from, Presence.composing)
			reply("Olha eu aqui")
	}
		...
		if (budy.includes(`Thanks`)) {
                  ...
                  if (budy.includes(`thanks`)) {
                  reply(`ꜱᴀᴍᴀ-ꜱᴀᴍᴀ ᴋᴀᴋ *${pushname2}*, Mk sᴀᴜᴅᴀᴅᴇs >-<`)
                  }
                  if (budy.includes(`makasih`)) {
                  reply(`ꜱᴀᴍᴀ-ꜱᴀᴍᴀ ᴋᴀᴋ *${pushname2}*, Mk sᴀᴜᴅᴀᴅᴇs >-<`)
                  }
                  ...
                  reply(`ᴏʟᴀ ᴊᴏᴠᴇᴍ *${pushname2}*, Mk ᴘᴏᴅᴇ sᴇʀ ᴀᴊᴜᴅᴀᴅᴏ? ᴅɪɢɪᴛᴇ *${prefix}menu* ʏᴀ ᴋᴀᴋ`)
                  }
			      if (budy.includes(`Dns`)) {
                  reply(`ᴏʟᴀ ᴊᴏᴠᴇᴍ*${pushname2}*, Mk ᴘᴏᴅᴇ sᴇʀ ᴀᴊᴜᴅᴀᴅᴏ? ᴅɪɢɪᴛᴇ *${prefix}menu* ʏᴀ ᴋᴀᴋ`)
                  }
